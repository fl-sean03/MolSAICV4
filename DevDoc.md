USM-Based MolSAIC V3 — Design and Development Plan

Summary
MolSAIC V3 replaces plugin registries, YAML DAGs, and executor indirection with a code-first pattern that mirrors this repository’s Unified Structure Model library and workspaces. The core becomes a small, dependable Python library for structure IO and deterministic ops, and pipelines are simple, reproducible scripts under workspaces with minimal JSON configs.

Status
This document serves as the authoritative plan for V3 implementation. The USM reference in this repository is the baseline for data model, IO, and core operations.
...
- IO modules: [src/usm/io/car.py](src/usm/io/car.py), [src/usm/io/mdf.py](src/usm/io/mdf.py), [src/usm/io/pdb.py](src/usm/io/pdb.py)
...
V3 consists of a small Python library plus a set of workspace scripts that call it.
...
- USM IO
  ...
  - See [src/usm/io/car.py](src/usm/io/car.py), [src/usm/io/mdf.py](src/usm/io/mdf.py), [src/usm/io/pdb.py](src/usm/io/pdb.py)
- USM Ops
...
V3 Package Naming & Import Strategy

- Distribution package: molsaicv3

- Source layout and imports
  - Core structures: import from usm.* (src/usm)
  - External wrappers and composition: import from external.* and pm2mdfcar.* (src/external, src/pm2mdfcar)

- Supported usage modes
  1) Editable install (recommended)
     - pip install -e 24-MOLSAIC-V3
     - Runners import normally (no sys.path hacks)
  2) Repo-local testing
     - Add 24-MOLSAIC-V3/src to sys.path for pytest (see tests), then imports work without modifying runners

- Canonical imports used by workspaces and tests
  - from external import msi2namd, packmol, msi2lmp → see [src/molsaicv3/usm/external/msi2namd.py](src/molsaicv3/usm/external/msi2namd.py), [src/molsaicv3/usm/external/packmol.py](src/molsaicv3/usm/external/packmol.py), [src/molsaicv3/usm/external/msi2lmp.py](src/molsaicv3/usm/external/msi2lmp.py)
  - import pm2mdfcar → see [src/molsaicv3/usm/ops/pm2mdfcar.py](src/molsaicv3/usm/ops/pm2mdfcar.py)
  - from usm.io.car import load_car, from usm.io.mdf import load_mdf

- CI guidance
  - For integration, see [tests/integration/test_hydration_workspaces.py](tests/integration/test_hydration_workspaces.py). Ensure an editable install or sys.path includes 24-MOLSAIC-V3/src before running pytest.
5. Repository layout
...
13. Migration guide from MolSAIC v1 to V3
...
Mapping examples
- fileio.parse mdf becomes load from [src/usm/io/mdf.py](src/usm/io/mdf.py) inside the script
- fileio.parse car becomes load from [src/usm/io/car.py](src/usm/io/car.py) inside the script
...
- Add V3 quickstart page under docs
